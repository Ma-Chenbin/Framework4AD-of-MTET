...
def estimate_tsen(datas,labs,dim,target_perplexity,plot=False):
    ...
    for m_iter in range(max_iter):
        ...
        if plot and m_iter % 100 == 0:
            print("Draw Map")
            draw_pic(Y,labs,name = "%d.png"%(m_iter))
    
    return Y
    

    
def draw_pic(datas,labs,name = '1.png'):
    plt.cla()
    plt.figure(figsize=(8, 8), dpi=300)
    plt.rc('font', family='Arial', size='26', weight='bold')  # 设置字体样式、大小
    plt.grid('true')
    plt.grid(color='b',
             linestyle='--',
             linewidth=2,
    ...
    for i in range(len(unque_labs)):
        ...
        pi = plt.scatter(datas[index, 0], datas[index, 1], s=100, c =[colors[i]])
        p.append(pi)
        legends.append(unque_labs[i])
        strs = [str(i) for i in legends]
        for i in range(len(strs)):
            if strs[i]=='0.0':
                strs[i]='Rest position'
            elif strs[i]=='1.0':
                strs[i]='Arm extension'
            elif strs[i]=='2.0':
                strs[i]='Wing-beat'
            elif strs[i]=='3.0':
                strs[i]='Finger-finger'
            elif strs[i]=='5.0':
                strs[i]='Finger-to-nose'
    ...
    plt.tight_layout()
    plt.xlabel('t-SNE 1', font)
    plt.ylabel('t-SNE 2', font)
    plt.legend(p, strs, prop = {'size':20,'family': 'Arial', 'weight': 'bold'}, loc='lower left')
...
if __name__ == "__main__":
    ...
    # mnist_datas = np.loadtxt("./mnist/mnist2500_X.txt")
    # mnist_labs = np.loadtxt("./mnist/mnist2500_labels.txt")
    mnist_datas = np.loadtxt("./mnist/data_s_array.txt")
    mnist_labs = np.loadtxt("./mnist/lab_s_array.txt")
    ...
    draw_pic(Y,mnist_labs,name = "final.png")
